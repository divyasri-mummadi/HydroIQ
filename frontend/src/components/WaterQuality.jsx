...
export default function WaterQuality() {
   ...
   * }
   *
   * Therefore we use zones[0].
   */

  const currentZone = sensorData?.zones?.[0] || sensorData || {};

  const ph = Number(currentZone?.ph);
  const tds = Number(currentZone?.tds);
  const turbidity = Number(currentZone?.turbidity);

  const waterQuality = analytics?.water_quality || {};

  /*
   * Calculate a simple quality score when the backend
  ...
  const getParameterStatus = (type) => {
    if (type === 'ph') {
      if (Number.isNaN(ph)) return 'UNKNOWN';
    ...
    if (type === 'tds') {
      if (Number.isNaN(tds)) return 'UNKNOWN';
    ...
    if (type === 'turbidity') {
      if (Number.isNaN(turbidity)) return 'UNKNOWN';
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Live measurements from the connected sensor node
            </p>
          ...
          <div className="flex justify-between items-start">
            ...
            <div>
              ...
              <h3 className="text-xl font-bold mt-2">
                {currentZone?.device_id || 'ESP32_Node_1'}
              </h3>

              <p className="text-sm text-gray-500 mt-1">
                Zone: {currentZone?.zone || 'Zone_A'}
              </p>
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1 leading-relaxed">
              ...
              {qualityStatus === 'POOR'
                ? 'Immediate water quality investigation is recommended. Verify the affected sensors and inspect the zone before normal operation continues.'
                : qualityStatus === 'WARNING'
                ? 'Continue close monitoring and investigate any parameter showing a sustained abnormal trend.'
                : 'Water quality is currently within the monitored range. Continue routine telemetry monitoring.'}
